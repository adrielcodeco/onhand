...
import { manageDocumentMetadata } from '@onhand/openapi/#/documentMetadata'
import { managePathMetadata } from '@onhand/openapi/#/pathMetadata'
...
export function getRoutes (openApiPath: string, srcPath: string): Route[] {
  ...
  for (const file of files) {
    ...
    for (const _exportKey in module) {
      ...
      const metadata = managePathMetadata(_export).get()
      if (metadata) {
